...
pub(super) enum RowKey {
    /// One stage.
    Stage(Stage),
    /// One repository section in one stage.
    Repo(Stage, String),
    /// One task, by its stable task identifier.
    Ticket(String),
    /// One release train.
    Train(String),
...
pub(super) fn selection_key(state: &StateView, selection: Selection) -> Option<RowKey> {
    let Selection::Row(index) = selection else {
        return None;
    };
    let all = rows(state);
    row_key(state, all.get(index)?)
}

/// Resolve one stable key in a new state view.
pub(super) fn selection_for_key(state: &StateView, key: &RowKey) -> Selection {
    rows(state)
        .iter()
        .position(|row| row_key(state, row).as_ref() == Some(key))
        .map_or(Selection::None, Selection::Row)
}

/// The stable key of one logical board item.
fn row_key(state: &StateView, row: &Row) -> Option<RowKey> {
    match row {
        Row::Stage { stage } => Some(RowKey::Stage(*stage)),
        Row::Repo { stage, repo } => Some(RowKey::Repo(*stage, repo.clone())),
...
///
/// A train in flight owns a release task. That task does not always run.
/// It can wait in the queue. It can also end while the train stays in
/// flight. `Train::finish` keeps the train when a label call fails. A
/// `Done` task can therefore hold the train through every retry.
///
/// The title therefore names the two live states and nothing else. It
/// reports the task, not the train. The board then never claims a release
/// that no agent runs. The task row inside the box shows the exact state.
///
...
/// today, because only the refine stage sets that state. A release that
/// parks needs its own title here. The reaper stops the process of a
/// parked session and leaves the state behind.
